...
export function setAmount(ingredient: IngredientCostCalculatorModel, amount: number): IngredientCostCalculatorModel{
    return {...ingredient, ingredient_amount: amount, products: ingredient.products.map(p=>{return{
        ...p,weight: calcIngredientSourceWeight(ingredient)*p.raw_content_percentage/100
    }})}
}

export function calcIngredientCost(ingredient: IngredientCostCalculatorModel):IngredientCostCalculatorModel{

    // const sourceWeight = calcIngredientSourceWeight(ingredient)
...
export function calcIngredientSourceWeight(ingredient: IngredientDTO){
    ...
    return ingredient.item_weight*(ingredient.ingredient_amount??1) / (100 - avgWastePercentage) * 100
}