...
export function calcAvgWastePercentage(ingredient: IngredientDTO){
    const avg = (ingredient.products?.reduce(
        (result, current)=>
            (current.waste_percentage&&current.raw_content_percentage)?result+current.waste_percentage*current.raw_content_percentage:0, 0
    )??NaN)/100
    return avg
}
...
export function getSourceWeight(calcState: IngredientProductsWeightsCalculatorState, ingredientAmount: number) : number {
    return (ingredientAmount * calcState.ingredient.item_weight) / (100 - calcState.avgWastePercentage) * 100
}
